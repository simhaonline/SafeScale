...
func (s *Stack) findVMByIDS(id string) (govcd.VM, fail.Error) {
	...
	vapp, nerr := vdc.FindVAppByID(id)
	if nerr != nil {
		return govcd.VM{}, nil
	}

	vmg := vapp.VApp.Children.VM[0].Name

	vm, nerr := vdc.FindVMByName(vapp, vmg)
	if nerr != nil {
		return govcd.VM{}, normalizeError(nerr)
	}
...
func (s *Stack) findDiskByName(name string) (*govcd.Disk, fail.Error) {
	...
	if res.Results != nil {
		...
		for _, drec := range r.DiskRecord {
			if drec.Name == name {
				theDisk, ferr := vdc.FindDiskByHREF(drec.HREF)
				if ferr != nil {
					return nil, normalizeError(ferr)
				}
				return theDisk, nil
			}
		}
...
func (s *Stack) getPublicIPs() (types.IPRanges, error) {
	...
	eg, nerr := vdc.FindEdgeGateway(names[0])
	if nerr != nil {
		return types.IPRanges{}, normalizeError(nerr)
	}

	gins := eg.EdgeGateway.Configuration.GatewayInterfaces
	for _, gif := range gins.GatewayInterface {
		for _, part := range gif.SubnetParticipation {
			if part.IPRanges != nil {
				return *part.IPRanges, nil
			}
		}
	}
...
func (s *Stack) CreateNetwork(req abstract.NetworkRequest) (network *abstract.Network, xerr fail.Error) {
	...
	logrus.Debug("vclouddirector.Client.CreateNetwork() called")
	defer logrus.Debug("vclouddirector.Client.CreateNetwork() done")
	...
	ran, nerr := toValidIPRange(req.CIDR)
	if nerr != nil {
		return nil, normalizeError(nerr)
	}
...
func (s *Stack) GetNetworkByName(ref string) (*abstract.Network, fail.Error) {
	...
	onet, nerr := vdc.FindVDCNetwork(ref)
	if nerr != nil {
		if strings.Contains(nerr.Error(), "can't find") {
			return nil, abstract.ResourceNotFoundError("network", ref)
		} else {
			return nil, normalizeError(nerr)
		}
...
func (s *Stack) ListNetworks() ([]*abstract.Network, fail.Error) {
	...
	// TODO: use concurrency.Tracer
	logrus.Debug("vclouddirector.Client.ListNetworks() called")
	defer logrus.Debug("vclouddirector.Client.ListNetworks() done")
...
func (s *Stack) DeleteNetwork(ref string) fail.Error {
	...
	// TODO: use concurrency.Tracer
	logrus.Debug("vclouddirector.Client.DeleteNetwork() called")
	defer logrus.Debug("vclouddirector.Client.DeleteNetwork() done")
