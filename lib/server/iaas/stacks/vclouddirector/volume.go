...
func (s *Stack) CreateVolume(request abstract.VolumeRequest) (*abstract.Volume, fail.Error) {
	...
	diskCreateParams := &types.DiskCreateParams{Disk: &types.Disk{
		Name:       request.Name,
		Size:       int64(int(request.Size) * 1024 * 1024 * 1024),
		BusType:    "6",
...
func (s *Stack) ListVolumes() ([]abstract.Volume, fail.Error) {
	...
	for _, ref := range refs {
		...
		if err == nil {
			thed, err := vdc.FindDiskByHREF(dr.Disk.HREF)
			if err == nil {
				volumes = append(volumes, abstract.Volume{Name: ref.Name, ID: ref.ID, Size: int(thed.Disk.Size)})
			}
		}
	}
...
func (s *Stack) CreateVolumeAttachment(request abstract.VolumeAttachmentRequest) (string, fail.Error) {
	...
	attask, nerr := vm.AttachDisk(&types.DiskAttachOrDetachParams{Disk: &types.Reference{HREF: disk.Disk.HREF}})
	if nerr != nil {
		return "", normalizeError(nerr)
	}

	nerr = attask.WaitTaskCompletion()
	if nerr != nil {
		return "", normalizeError(nerr)
	}
...
func (s *Stack) DeleteVolumeAttachment(serverID, id string) fail.Error {
	...
	attask, nerr := vm.DetachDisk(&types.DiskAttachOrDetachParams{Disk: &types.Reference{HREF: disk.Disk.HREF}})
	if err != nil {
		return normalizeError(nerr)
	}
