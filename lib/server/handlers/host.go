...
func (handler *HostHandler) Create(
	...
	if err != nil {
		...
		if client.IsProvisioningError(derr) {
			...
			return nil, scerr.Wrap(derr, fmt.Sprintf("failed to provision host '%s', please check safescaled logs", host.Name))
		}
	...
	if retcode != 0 {
		...
		return nil, scerr.Errorf(fmt.Sprintf("failed to finalize host '%s' installation: retcode=%d, stdout[%s], stderr[%s]", host.Name, retcode, stdout, stderr), nil)
	}
	...
	if err != nil {
		...
		if client.IsProvisioningError(err) {
			retrieveForensicsData(ctx, sshHandler, host)
			return nil, scerr.Wrap(err, fmt.Sprintf("error creating host '%s', error provisioning the new host, please check safescaled logs", host.Name))
		}
...
func (handler *HostHandler) Delete(ctx context.Context, ref string) (err error) {
	...
	err = host.Properties.LockForRead(hostproperty.SharesV1).ThenUse(func(clonable data.Clonable) error {
		...
		for _, share := range shares {
			...
			if count > 0 {
				count = len(shares)
				return scerr.Errorf(fmt.Sprintf("cannot delete host, exports %d share%s where at least one is used", count, utils.Plural(count)), nil)
			}
		}
		return nil
	})
	...
	err = host.Properties.LockForRead(hostproperty.VolumesV1).ThenUse(func(clonable data.Clonable) error {
		...
		if nAttached > 0 {
			return scerr.Errorf(fmt.Sprintf("host has %d volume%s attached", nAttached, utils.Plural(nAttached)), nil)
		}
		return nil
	})
	if err != nil {
	...
	err = host.Properties.LockForRead(hostproperty.NetworkV1).ThenUse(func(clonable data.Clonable) error {
		if clonable.(*propsv1.HostNetwork).IsGateway {
			return scerr.Errorf(fmt.Sprintf("cannot delete host, it's a gateway that can only be deleted through its network"), nil)
		}
		return nil
	})
	if err != nil {
	...
	if deleteMetadataOnly {
		return scerr.Errorf(fmt.Sprintf("unable to find the host even if it is described by metadata. Dirty metadata have been deleted"), nil)
	}
		...
		err2 := host.Properties.LockForRead(hostproperty.SizingV1).ThenUse(func(clonable data.Clonable) error {
			...
			return host.Properties.LockForRead(hostproperty.DescriptionV1).ThenUse(func(clonable data.Clonable) error {
				...
				return host.Properties.LockForRead(hostproperty.NetworkV1).ThenUse(func(clonable data.Clonable) error {
					...
					if err3 != nil {
						return scerr.Errorf(fmt.Sprintf("failed to stop host deletion : %s", err3.Error()), err3)
					}
					return nil
				})
			})
